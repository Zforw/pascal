...
void nfa_dfa(string file_name) {
	...
	cout << endl;
	for (int i = 0; i < start.size(); i++) {
		trans[*start[i].begin()][_action[i] % 'a'] = *final[i].begin();
	...
	for (int i = 0; i < start_minDFA.size(); i++) {
		cout << start_minDFA[i] << "-> " << action_minDFA[i] << "->" << final_minDFA[i] << " ";
		if (flag_minDFA[i] == true) {
			cout << "终态" << endl;
		}
		else
			cout << "非终态" << endl;
